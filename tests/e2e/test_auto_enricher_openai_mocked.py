...
def color_response_from_prompt(**kwargs):
    ...
    return fake_openai_response(ColorEnrich(color=color))
...
def test_auto_enricher_repeated_prompt_uses_cache(mock_key_for_provider, mock_openai):
    ...
    mock_client.responses.parse.side_effect = [
        fake_openai_response(ColorEnrich(color="blue")),
        fake_openai_response(ColorEnrich(color="red")),
    ]
    mock_openai.return_value = mock_client
...
def test_auto_enricher_enrich_all_basic(mock_key_for_provider, mock_openai):
    ...
    mock_client = Mock()
    mock_client.responses.parse.side_effect = color_response_from_prompt
    mock_openai.return_value = mock_client
    ...
    assert mock_client.responses.parse.call_count == len(prompts)
...
def test_auto_enricher_enrich_all_handles_multiple_batches(mock_key_for_provider, mock_openai):
    ...
    mock_client = Mock()
    mock_client.responses.parse.side_effect = record_and_respond
    mock_openai.return_value = mock_client
