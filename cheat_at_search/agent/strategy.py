...
class ReasoningSearchStrategy(SearchStrategy):
    ...
    def __init__(
        self, corpus, search_client: Agent, prompt: str, cache=True, workers=1
    ):
        ...
        if cache:
            ...
            self.cache_path = (
                cached_results_dir / f"reasoning_search_cache_{self.hash}.pkl"
            )
            self.cache = {}
            print(f"Cache enabled at {self.cache_path}")
            try:
                with open(self.cache_path, "rb") as f:
                    self.cache = pickle.load(f)
                    print(
                        f"Loaded {len(self.cache)} cached search results from",
                        self.cache_path,
                    )
    ...
    def search(self, query, k=10):
        ...
        resp, inputs, total_tokens = self.search_client.chat(prompt, return_usage=True)
        ...
        if self.cache is not None:
            ...
            print(f"Cache write for query: {query}")
        else:
            print("Not caching")
        return top_k, scores
